...
from resnext import resnext_layer, resnext_block
...
def soft_attention_mask(input_layer, num_filters, is_training):
    ...
    upscale1 = tf.image.resize_images(res3, [input_size // 2, input_size // 2])
...
def attention_model(input_layer, is_training):
    ...
    # pool into one thing
    output = tf.nn.avg_pool(x,[1, 8, 8, 1],strides=[1,8,8,1],padding='VALID')
    return output
